...
@Path("pessoas")
@RequestScoped
@Api("Endpoints SGM Prefeitura Bom Destino")
@WebFilter(asyncSupported = true)
public class PMBDResources {

    @EJB
    private Service service;

    @Inject
    Event<PessoaFisica> pessoaEvent;
    ...
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    public Response cadastrar(PessoaFisica pessoa) {
...
//            pessoaEvent.fire(pessoa);
    ...
    @PUT
    @Produces(MediaType.APPLICATION_JSON)
    public Response atualizar(PessoaFisica pessoa) {
        ResponseModel responseModel = new ResponseModel();
    ...
    public Response buscarPorID(
        ...
        ResponseModel responseModel = new ResponseModel();
        try {
            PessoaFisica pessoa = service.buscarPorId(id);
            if (pessoa == null) {
                return Response.status(Response.Status.NOT_FOUND).entity(responseModel).build();
            }
    ...
    public Response buscar(@QueryParam("nome") String nome,
        ...
        try {
            responseModel.getData().add(service.buscarTodos(nome, cpf, email, null));
            responseModel.getMessages().add(new Message(200, "OK!"));
        } catch (APIException ex) {
            responseModel.getMessages().add(new Message(ex.getCodigo(), ex.getDescricao()));
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(responseModel).build();
